...
// Danh sách các loại phụ tùng cố định
const CATEGORIES = [
  "Lọc dầu",
  "Lọc gió",
  "Má phanh",
...
const UserOrderInterface = () => {
  ...
  const fetchCarParts = async () => {
    setLoading(true);
    try {
      // Sửa lại cách gọi phân trang theo cách Strapi hoạt động
      const queryParams = new URLSearchParams();
      queryParams.append('pagination[start]', (page - 1) * 12);
      queryParams.append('pagination[limit]', 12);
      
      if (searchQuery) {
        queryParams.append('filters[name][$containsi]', searchQuery);
      }
      
      if (selectedCategory) {
        queryParams.append('filters[category][$eq]', selectedCategory);
      }
      ...
      setTotalPages(Math.ceil(response.meta.pagination.total / response.meta.pagination.limit));
      setError(null);
    } catch (err) {
  ...
  // Loại bỏ hàm fetchCategories vì sử dụng danh mục cố định
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        orderNumber,
        user: 1, // Giả định đã có thông tin user từ context hoặc API
        car_part: selectedPart.id, // Sửa từ carPart thành car_part
        quantity,
